...
func FaviconIVG() ([]byte, error) {
	...
	// The favicon graphic also uses a dark version of that base color.
	dark := color.RGBA{0x23, 0x1d, 0x1b, 0xff}

	// Blend is 75% dark (CReg[63]) and 25% the base color (pal[0]).
	// t = 64 / 256 = 0.25
	// c0 = 255 -> CREG color at index (255 - 192) = 63
	// c1 = 128 -> Custom Palette color at index (128 - 128) = 0
