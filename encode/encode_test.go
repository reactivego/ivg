...
func TestEncodeArcs(t *testing.T) {
	...
	e.SetCReg(1, false, ivg.RGBAColor(color.RGBA{0xff, 0x00, 0x00, 0xff}))
	e.SetCReg(2, false, ivg.RGBAColor(color.RGBA{0xff, 0xff, 0x00, 0xff}))
	e.SetCReg(3, false, ivg.RGBAColor(color.RGBA{0x00, 0x00, 0x00, 0xff}))
	e.SetCReg(4, false, ivg.RGBAColor(color.RGBA{0x00, 0x00, 0x80, 0xff}))
...
func TestEncodeFavicon(t *testing.T) {
	...
	var e Encoder
	e.Reset(ivg.DefaultViewBox, &pal)
	...
	// color with pal[0].
	e.SetCReg(1, false, ivg.RGBAColor(dark))
		...
		if adj >= 2 {
			if !seenFCI2 {
				...
				for i, c := range remainingColors {
					e.SetCReg(uint8(i), false, ivg.RGBAColor(c))
				}
			}
			adj -= 2
		}
...
func TestEncodeVideo005Primitive(t *testing.T) {
	...
	for _, v := range video005PrimitiveSVGData {
		e.SetCReg(0, false, ivg.RGBAColor(color.RGBA{
			uint8(v.r * 128 / 255),
			uint8(v.g * 128 / 255),
			uint8(v.b * 128 / 255),
			128,
